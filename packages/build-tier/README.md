...
- No `.d.ts` mirror — and this is deliberate, not a gap. The JSDoc tag survives into `.d.ts` natively (IDE hover shows it); the resolved `tier` value lives in the JS runtime output only. Putting the resolved `tier` on the *type-system* path would only earn its keep if something consumed tier at the type level — and nothing does: tier filtering happens at runtime (`registry.list({ tiers })`, the MCP / AI adapters). Without a concrete type-level consumer named, a `.d.ts` mirror would be speculative polish; it can ship when a real consumer surfaces.
