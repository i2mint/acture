...
2. **Which surfaces** — chose **hotkeys + MCP + AI**, the three surfaces with shipping packages (`acture-hotkeys`, `acture-mcp-server`, `acture-ai-vercel`). **telemetry / undo / extensions were deliberately excluded:** they have no shipping packages (telemetry & undo are post-v1, rule-of-three gated), so their consumer skills would be agent-written-path-only — heavier, less consistent with the palette/macros/e2e template, and better as a later increment if/when those packages exist.
...
Three skills, all building on `acture-consumer-integration`, all following the `acture-macros` / `acture-e2e` template (the "load the foundation first" callout, the two-decisions framing, the "what to get right" list, the "what NOT to build" rule-of-three section, grounded see-also links):
